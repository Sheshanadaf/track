...
const AccountScreen = () => {
  ...
  const getUserDetails = async () => {
    try {
      const response = await trackerApi.get(`/getUserDetails`);
  ...
  useEffect(() => {
    if (isFocused) {
      getUserDetails(); // Fetch data when screen is focused
    }
  }, [isFocused]); 
    ...
    <ImageBackground source={require('../../../assets/acc.jpg')} style={{ height: 800, paddingTop:0 }}>
      ...
      <View style={{ paddingTop: 150 }}>
        ...
        <View style={{paddingLeft:30, paddingTop:20, paddingBottom:40}}>
          ...
          <Text style={{fontSize:18}}>Register Number :  {regNum}</Text>
          <View style={{padding:2}}></View>
          <Text style={{fontSize:18}}>Batch Codes :  {batchCode}</Text>
